/// Ayame WebRTC sample — connects this Flutter app to an Ayame signaling
/// server, joins a 1-on-1 room, and runs a bidirectional video call.
///
/// Protocol: https://github.com/OpenAyame/ayame-spec
///
/// On launch the app shows a small form for the WebSocket URL, the room
/// id, and the optional signaling key; the defaults can be pre-filled via
/// `--dart-define=AYAME_URL=...` (likewise `AYAME_ROOM`, `AYAME_SIGNALING_KEY`).
/// Tapping "Join" connects, registers, and exchanges an H.264 video
/// stream with the other peer in the room.
///
/// Run (macOS):
/// ```
/// cd flutter/example
/// flutter run -d macos -t lib/ayame_main.dart \
///   --dart-define=AYAME_URL=ws://localhost:3000/signaling
/// ```
library;
...
class _ConfigFormState extends State<_ConfigForm> {
  static const String _defaultUrl =
      String.fromEnvironment('AYAME_URL',
...
class _CallViewState extends State<_CallView> {
  ...
  Future<void> _start() async {
    ...
    stdout.writeln('[ayame] ws connected room=${widget.config.room} '
        'client=$_clientId');
    if (mounted) {
    ...
    _ws!.listen(_onSignalingMessage,
        onDone: () => stdout.writeln('[ayame] ws closed'),
        onError: (Object e) => stderr.writeln('[ayame] ws error: $e'));
  }
  ...
  Future<void> _onSignalingMessage(dynamic data) async {
    ...
    switch (msg['type'] as String?) {
      ...
      case 'reject':
        ...
        stderr.writeln('[ayame] rejected: $reason');
      ...
      case 'bye':
        stdout.writeln('[ayame] peer left');
  ...
  Future<void> _onAccept(Map<String, dynamic> msg) async {
    ...
    stdout.writeln('[ayame] accepted (offerer=$_isOfferer, '
    ...
    pc.onTrack.listen((evt) {
      ...
      stdout.writeln('[ayame] onTrack ssrc=${evt.ssrc}');
      _wireIncomingTrack(evt.receiver);
    });

    pc.onConnectionStateChange.listen((s) {
      stdout.writeln('[ayame] PC: $s');
    ...
    if (_isOfferer) {
      ...
      stdout.writeln('[ayame] sending offer');
  ...
  Future<void> _onRemoteSdp(
    ...
    if (type == SessionDescriptionType.offer) {
      ...
      stdout.writeln('[ayame] sending answer');
  ...
  void _startSendingVideo(RtpSender sender) {
    ...
    _encoder = VideoEncoder(
      ...
      error: (e) => stderr.writeln('[ayame] encoder error: $e'),
    );
  ...
  void _wireIncomingTrack(RtpReceiver receiver) {
    ...
    _decoder = VideoDecoder(
      ...
      error: (e) => stderr.writeln('[ayame] decoder error: $e'),
    );
