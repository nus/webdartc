...
final class DtlsV13ClientStateMachine implements core.ProtocolStateMachine {
  ...
  /// The most recently emitted client flight (CH1, CH2, or final flight),
  /// kept so [handleTimeout] can re-send it when the peer doesn't reply
  /// in time (RFC 9147 §5.7).
  List<OutputPacket>? _lastClientFlight;
  ...
  core.Result<ProcessResult, core.ProtocolError> startHandshake({
    ...
    _lastClientFlight = List.of(flight);
  ...
  core.Result<ProcessResult, core.ProtocolError> handleTimeout(
    ...
    // Retransmit only while still in a handshake state; once connected
    // or failed the timer is irrelevant.
    if (_state == DtlsV13ClientState.connected ||
        _state == DtlsV13ClientState.failed ||
        _state == DtlsV13ClientState.initial) {
      return const core.Ok(ProcessResult.empty);
    }
    final flight = _lastClientFlight;
  ...
  /// Exponential-backoff timeout for the active client flight.
  ...
  core.Result<ProcessResult, core.ProtocolError> _processHandshakeFragments(
    ...
    while (offset < buf.length) {
      ...
      // Propagate any retransmit timeout the inner handler set so the
      // upper layer can schedule it (RFC 9147 §5.7).
