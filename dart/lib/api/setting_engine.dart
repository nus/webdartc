...
/// [PeerConnection]s created by a [Webdartc].
///
/// Covers knobs that the W3C [PeerConnectionConfiguration] does not
/// expose because they are implementation-level (specific bind
/// addresses, ephemeral port ranges, interface filters). Construct
/// with named arguments and `const` whenever possible:
///
/// ```dart
...
final class SettingEngine {
  ...
  /// ephemeral port (Dart `bind(addr, 0)` semantics).
  ...
  /// interface from candidate gathering. Cannot participate in `const`
  /// construction (closures are not const-compatible); the
  /// [SettingEngine] holding a non-null filter is therefore non-const.
