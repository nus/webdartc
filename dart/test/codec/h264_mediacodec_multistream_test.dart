/// Verifies that multiple `MediaCodecEncoderBackend` + `MediaCodecDecoderBackend`
/// instances run simultaneously without cross-talk. Each pair owns its own
/// `AMediaCodec`, so a regression that shared state across instances would
/// surface here as cross-stream PTS interleaving or frame corruption.
///
/// MediaCodec only exists on Android, so this gates itself off host `dart test`
/// (where it would have no `libmediandk.so`). It runs ON-DEVICE through the
/// Flutter integration aggregator
/// (`flutter/example/integration_test/dart_suite_test.dart`), which calls this
/// file's `main()` directly under an Android emulator/device.
@Tags(['native'])
@TestOn('android')
library;
...
void main() {
  ...
  test('3 MediaCodec encoder+decoder pairs interleaved, 20 frames each',
      () async {
    const numStreams = 3;
  ...
  test('3 MediaCodec encoder+decoder pair lifecycles back-to-back', () async {
    for (var s = 0; s < 3; s++) {
