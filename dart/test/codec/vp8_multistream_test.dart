/// Verifies that multiple `Vp8EncoderBackend` + `Vp8DecoderBackend`
/// instances can run simultaneously without cross-talk. Each pair owns
/// its own `vpx_codec_ctx_t`, but a regression in the wrapper (shared
/// state, leaked iterators, scratch-buffer aliasing) would be caught
/// here before it shows up as cross-stream frame corruption in a
/// realistic SFU client.
@Tags(['native'])
library;
...
class _Stream {
  ...
  void step(int frameIndex) {
    ...
    // FakeVideoSource.frameAt() always tags timestamp=0, so rebuild with
    // a real PTS to verify per-frame round-trip ordering.
...
void main() {
  ...
  test('4 VP8 encoder+decoder pairs interleaved, 30 frames each', () async {
    const numStreams = 4;
  ...
  test('5 VP8 encoder+decoder pair lifecycles back-to-back', () async {
    for (var s = 0; s < 5; s++) {
