/// Verifies that multiple MediaCodec VP9 encoder + decoder instances run
...
///
/// The libvpx VP9 multistream test (`vp9_multistream_test.dart`) covers the
/// bundled encoder used off-Android; this is the MediaCodec analogue and runs
/// where `registerVp9Codec` selects MediaCodec (Android).
///
...
void main() {
  ...
  test('3 MediaCodec VP9 encoder+decoder pairs interleaved, 20 frames each',
      () async {
    const numStreams = 3;
  ...
  test('3 MediaCodec VP9 encoder+decoder pair lifecycles back-to-back',
      () async {
    for (var s = 0; s < 3; s++) {
