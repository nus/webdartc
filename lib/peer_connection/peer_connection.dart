...
final class PeerConnection {
  ...
  Future<SessionDescription> createOffer() async {
    ...
    if (_transceivers.isNotEmpty) {
      ...
      final tracks = _transceivers.map((t) {
        if (t.kind == 'audio') {
          return MediaTrack(type: 'audio', direction: t.direction, codecs: [
  ...
  /// Assign MID values and extension IDs to senders from the remote offer SDP.
  void _assignMidToSenders(SdpSessionDescription remoteOffer) {
    // Find the MID extension ID from the SDP extmap.
    int midExtId = 0;
    for (final m in remoteOffer.media) {
      for (final extmap in m.getAll('extmap')) {
    ...
    if (midExtId == 0) return;

    // Match offer m-lines to our transceivers by kind and assign MID.
    int tIdx = 0;
    for (final m in remoteOffer.media) {
      if (m.type == 'application') continue;
      final mid = m.mid ?? '${remoteOffer.media.indexOf(m)}';
